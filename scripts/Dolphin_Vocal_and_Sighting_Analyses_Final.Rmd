...
dolphinz <- read.csv("./data/bimonthly.csv")
...
summary(multtot_lm)
plot(multtot_lm)
#R squared  = 0.535, Adjusted = 0.4651
stepAIC(multtot_lm)
#remove noiseratio and toadfish
#new R squared  = 0.535, Adjusted = 0.4761
...
```{r}
#Beta coefficients for partial correlation plots
tst <- lm((log1p(total)) ~ site + season, dolphinz)
tot_parcor <- lm(scale(residuals(tst)) ~ scale(dolphin), data = dolphinz)
tot_parcor

dolphinz_std <- dolphinz%>%mutate_if(is.numeric,scale)
#scaling to standardize numeric variables

multtot_lm <- lm(log1p(total) ~ dolphin + site + season + drum + perch + trout, data = dolphinz)
multtot_lmstd <- lm(log1p(total) ~ dolphin + site + season + drum + perch + trout, data = dolphinz_std)

round(summary(multtot_lm)$coef, 6)
round(summary(multtot_lmstd)$coef, 6)
#comparing Beta coeffs between model with and without scaling
#t stat and p-value aren't supposed to change at all and they are, could be because of categorical variables?

cor(log1p(dolphinz$total), dolphinz$dolphin)
cor(log1p(dolphinz$total), dolphinz$drum)
cor(log1p(dolphinz$total), dolphinz$perch)
cor(log1p(dolphinz$total), dolphinz$trout)
#individual correlation coefficients not very similar to the Beta estimates in the scaled model
...
pairs(dolphinz, lower.panel = panel.cor, upper.panel = panel.smooth)
#pairs plot looks pretty crazy due to the addditional variables in the dataframe we aren't including

newdf_totpairs <- dolphinz[c("total","dolphin","site", "season", "drum", "perch", "trout")]
#new dataframe only including model variables

newdf_totpairs$logtotal <- log1p(newdf_totpairs$total)

pairs(newdf_totpairs, lower.panel = panel.cor, upper.panel = panel.smooth)
#looks like site and season are colinear with red drum but I'm not sure how this would affect Beta coefficients
#added log+1(total) variable in dataframe since I am using this transformation in my model and can now see strong
#colinearity between log+1(total) and dolphin and log+1(total) and site, multicolinearity problem? 

#decided to not moved forward with reporting standardize Beta coefficients for partial correlation coefficients 
```
...
```{r}
#pseudo R-squared
with(summary(elog.glm), 1 - deviance/null.deviance)
```
```{r}
Anova(elog.lm)
```
...
BP - GLM Model
```{r}
bplog.lm <- lm(log1p(bp) ~ dolphin + site + season + site*season + drum + trout, data = dolphinz)
summary(bplog.lm)
...
```{r}
#pseudo R-squared
with(summary(bplog.glm), 1 - deviance/null.deviance)
```
```{r}
Anova(bplog.lm)
```
...
```{r}
whlog.lm <- lm(log1p(whist) ~ dolphin + site + season + site*season, data = dolphinz)
summary(whlog.lm)
...
```{r}
#pseudo R-squared
with(summary(whlog.glm), 1 - deviance/null.deviance)
```
